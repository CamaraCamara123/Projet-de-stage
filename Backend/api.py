...
# @token_required
# @login_required
def upload_patient_image(username):
...
# @login_required
# @admin_required
def delete_dermatologue(dermatologue_id):
    try:
...
# @login_required
# @admin_required
def update_dermatologue(dermatologue_id):
    data = request.get_json()
...
# @admin_required
# @secretaire_required
def update_secretaire(secretaire_id):
    data = request.get_json()
...
def delete_secretaire(secretaire_id):
    try:
        secretaire = Secretaire.objects.get(pk=secretaire_id)
    except Secretaire.DoesNotExist:
        return jsonify({"message": "Secrétaire non trouvé"}), 404
...
def create_rdv(patient_id, medecin_id):
    try:
        patient = Patient.objects.get(pk=patient_id)
    except Patient.DoesNotExist:
...
def get_rdv_by_patient(patient_id):
    try:
        patient = Patient.objects.get(pk=patient_id)
    except Patient.DoesNotExist:
...
def get_stades_by_Maladie(maladie_id):
    try:
        maladie = Maladie.objects.get(pk=maladie_id)
    except Maladie.DoesNotExist:
        return jsonify({"message": "maladie introuvable"}), 404
